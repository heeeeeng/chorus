...
func NewAngine(lgr *zap.Logger, conf *viper.Viper) (angine *Angine) {
	...
	// TODO delete dumpLgr related code
	logpath := viper.GetString("log_path")
	dumpLgr := log.Initialize("", path.Join(logpath, "dump.output.log"), path.Join(logpath, "dump.err.log"))
	p2psw, err := prepareP2P(logger, dumpLgr, conf, gb, privValidator, refuseList)
	if err != nil {
...
func prepareP2P(logger, dumpLogger *zap.Logger, conf *viper.Viper, genesisBytes []byte, privValidator *agtypes.PrivValidator, refuseList *refuse_list.RefuseList) (*p2p.Switch, error) {
	p2psw := p2p.NewSwitch(logger, dumpLogger, conf, genesisBytes)
